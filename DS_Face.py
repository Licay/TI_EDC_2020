...
def RunFirst(task_fd):
    a = kpu.init_yolo2(task_fd, 0.5, 0.3, 5, anchor) #初始化人脸检测模型


def RunOnce(img,task_fd,task_ld,task_fe,key_pressed): # 主循环
    global record_ftrs
    global record_ftr
    ...
    if code: # 如果检测到人脸
        for i in code: # 迭代坐标框
            ...
            feature=kpu.face_encode(fmap[:]) #获取计算结果z
            reg_flag = False
            scores = [] # 存储特征比对分数
            ...
            if max_score > 80: # 如果最大分数大于85， 可以被认定为同一个人
                a = img.draw_string(i.x(),i.y(), ("%s :%2.1f" % (names[index], max_score)), color=(0,255,0),scale=1.5) # 显示人名 与 分数
            else:
                a = img.draw_string(i.x(),i.y(), ("X :%2.1f" % (max_score)), color=(255,0,0),scale=1.5) #显示未知 与 分数
            if key_pressed == 1: #如果检测到按键
                #key_pressed = 0 #重置按键状态
                record_ftr = feature
                record_ftrs.append(record_ftr) #将当前特征添加到已知特征列表
                '''
                path = ("/sd/image_%1d.jpg")%(k)
                ff1 = open('/sd/face_ftrs.txt','w')
...
def TestDS_Face():
    '''
    task_fd = kpu.load(0x800000) # 从sd卡加载人脸检测模型
    task_ld = kpu.load(0x880000) # 从sd卡加载人脸五点关键点检测模型
    task_fe = kpu.load(0x900000) # 从sd卡加载人脸196维特征值模型
    '''
    '''
    #mask.kmodel    1.52M    # 从flash 0x600000 加载口罩识别模型
    #task_md = kpu.load_flash(0x600000, 1, 0xC000, 60000000)
    #face.smodel    379K     # 从flash 0x800000 加载人脸检测模型
    task_fd = kpu.load_flash(0x800000, 0, 0, 60000000)
    #five.smodel    245K     # 从flash 0x880000 加载人脸五点关键点检测模型
    task_ld = kpu.load_flash(0x880000, 0, 0, 60000000)
    #f196w.smodel   2.24M    # 从flash 0x900000 加载人脸196维特征值模型
    task_fe = kpu.load_flash(0x900000, 0, 0, 60000000)

    '''
    #task_md = kpu.load("/sd/mask.kmodel")  #加载口罩识别模型
    ...
    while True:
        key_val=not key_gpio.value()
        img = sensor.snapshot() #从摄像头获取一张图片
        clock.tick() #记录时刻，用于计算帧率
        print(RunOnce(img,task_fd,task_ld,task_fe,key_val))
        fps =clock.fps() #计算帧率
        #print("%2.1f fps"%fps) #打印帧率
        _=img.draw_string(2,2, ("%2.1ffps" %(fps)), color=(0,128,0), scale=1.5)
...
if __name__ == '__main__':
    print('this is DS_Face test')
    TestDS_Face()
else:
    print('import DS_Face')
